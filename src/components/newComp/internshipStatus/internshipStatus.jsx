import { Checkbox, Col, Menu, Row, Select, Tabs } from "antd";
...
export default function InternshipStatus(props) {
  ...
  const pendingApplication = fullList
    ? fullList.pending.map((application, index) => (
        <ApplicationCard
          myFun={myFun}
          key={index}
  ...
  const shortlistedApplication = fullList
    ? fullList.shortlisted.map((application, index) => (
        <ApplicationCard
  ...
  const selectedApplication = internApplication
    ? internApplication.applications
        .filter(application => application.status === 302)
  ...
  const rejectedApplication = internApplication
    ? internApplication.applications
        .filter(application => application.status === 303)
        .map((application, index) => (
          <ApplicationCard
            myFun={myFun}
            // isSelectAll={isSelectAll}
  ...
  useEffect(() => {
    ...
    axios.get(urlForInternApplication).then(res => {
      console.log(
        "%c Internship Application",
        "color: darkblue; font-size: 25px"
      );
      console.log(res.data);
      const pending = res.data.applications
        .filter(app => app.status === 300)
        .sort(
          (a, b) =>
            new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        );
      const shortlisted = res.data.applications
        .filter(app => app.status === 301)
        .sort(
          (a, b) =>
            new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        );
      const selected = res.data.applications
        .filter(app => app.status === 302)
        .sort(
          (a, b) =>
            new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        );
      const rejected = res.data.applications
        .filter(app => app.status === 303)
        .sort(
          (a, b) =>
            new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        );
  ...
  const onChangeCheckboxShortlisted = e => {
    ...
    const allShortlistedCandidate = fullList.shortlisted.map(
      app => app.user._id
    );
    ...
    <div style={{ marginTop: "5rem", padding: "0 5rem" }}>
      <div style={{ width: "250px", margin: "auto", textAlign: "center" }}>
        <Menu
          ...
          selectedKeys={[current0]}
          mode="horizontal"
        >
          <Menu.Item key="posting">POSTING</Menu.Item>
          <Menu.Item key="recruit">RECRUTE</Menu.Item>
        </Menu>
      </div>
      ...
      {current0 === "recruit" ? (
              ...
              <Tabs defaultActiveKey="pending" onChange={callback}>
                <TabPane tab={`Pending(${noOfPending})`} key="pending">
                  <div className="second-tab" style={{ margin: "2rem 0" }}>
                    <Checkbox onChange={onChangeCheckboxPending}>
                      Select All
                    </Checkbox>

                    <div
                      className="clickable-options"
                    ...
                    <div className="clickable-options-2">
                      ...
                      <Select
                        ...
                        style={{ width: 120 }}
                        onChange={handleChangeOptionsinPending}
                      >
                ...
                <TabPane
                  tab={`Shortlisted(${noOfShortlisted})`}
                  key="shortlisted"
                >
                  <div className="second-tab" style={{ margin: "2rem 0" }}>
                    <Checkbox onChange={onChangeCheckboxShortlisted}>
                      Select All
                    </Checkbox>
                    <div
                      className="clickable-options"
                    ...
                    <div className="clickable-options-2">
                      ...
                      <Select
                        defaultValue="Most Recent"
                        style={{ width: 120 }}
                        onChange={handleChangeOptionsInShortlisted}
                      >
                        <Option value="most-recent">Most Recent</Option>
                        <Option value="resume-score">Resume Score</Option>
                        {/* <Option value="op3">option3</Option> */}
        ...
        <SingleInternship internship={singleInternship} />
      ) : (
