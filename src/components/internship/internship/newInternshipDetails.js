...
export default function NewInternshipDetails(props) {
    ...
    const totalStats = (array,status) => array.length ? array.filter(el => el.status === status*1 ).length : 0
        ...
        axios.get(url1)
            ...
            axios.get(url2)
                .then(res => {
                    ...
                    const pending = applications.filter(el => el.status === 300).sort((a,b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime())
                    const selected = applications.filter(el => el.status === 301).sort((a,b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime())
                    const shortlisted = applications.filter(el => el.status === 302).sort((a,b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime())
                    const rejected = applications.filter(el => el.status === 303).sort((a,b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime())
    ...
    const columns = [
        ...
        //   dataIndex: "name",
          key: "name",
          render: (record) => (
            <div  onClick={() => openResumeModal(record.id)}  className="name-and-img">
      ...
      const tableData = (array) => {
        return array ? array.map(({user: {firstName,city,resumeScore,imgUrl,_id}}, index) => {
            return {
                key: index + 1,
                name:firstName,
      ...
      const pendingApplicationData = appliedApplicationsData ? tableData(appliedApplicationsData.pending) : null
      const selectedApplicationData = appliedApplicationsData ?  tableData(appliedApplicationsData.selected) : null
      const shortlistedApplicationData = appliedApplicationsData ?  tableData(appliedApplicationsData.shortlisted) : null
      const rejectedApplicationData = appliedApplicationsData ?  tableData(appliedApplicationsData.rejected) : null
      ...
      const selectHandler = (value) =>{
        ...
        if(currentTab === 'pending') oldApplication = appliedApplicationsData.pending
        else if(currentTab === 'selected') oldApplication = appliedApplicationsData.selected
        else if(currentTab === 'shortlisted') oldApplication = appliedApplicationsData.shortlisted
        else  oldApplication = appliedApplicationsData.rejected

        if (value === 'resume-score')
            sortedApplication = oldApplication.sort((a,b) => b.user.resumeScore - a.user.resumeScore)
        else if (value === 'most-recent')
            sortedApplication = oldApplication.sort((a,b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime())

        // console.log('sortedApplication ', sortedApplication)
        setAppliedApplicationsData({...appliedApplicationsData, [value]: sortedApplication })
      }
    return (
        <Fragment>
               {clickedUserId ? <WorkerDetails isShow={isShowResume} isClose={isCloseResume} internshipId={internshipId} userId={clickedUserId} /> : null }  
        <div className="internship-details-block">
            {internship ? <div className="internship-summary-and-overview-block">
                <div className="summary-block">
                    <h1 className="heading">Business Development Internship</h1>
                    <div className="suumary-details">
                        <div>Stipend <br/> <span>&#8377; {internship.stipendType === 'negotiable' ? internship.minStipend + '-' + internship.maxStipend : internship.stipendType === 'unpaid' ? 'Unpaid' : internship.stipend }</span> </div>
                        <div>Deadline <br/> <span>{internship.applyBefore}</span> </div>
                        <div>Duration <br/> <span>{internship.duration}</span> </div>
                ...
                {applicationStats ? <div className="overview-block">
                    ...
                    <div className="details">
                        ...
                        <div className="single-detail">
                             <p>Shortlisted</p> <span>{applicationStats.shortlisted}</span>
                        </div>
                        <div className="single-detail">
            ...
            <div className="interns-application-block">
                <div className="heading">
                    ...
                    <div className="filter">
                        <p>Sort By </p>
                        <Select value={selectDefault} style={{ width: 120 }} 
                ...
                <Tabs onChange={tabHandler} defaultActiveKey={currentTab} className="tab" type="card">
                    <TabPane tab="Pending" key="pending" className="">
                        <Table className="applied-gigs" columns={columns} dataSource={pendingApplicationData} pagination={false} />
                    </TabPane>
                    <TabPane tab="Shortlisted" key="shortlisted" className="">
                        <Table className="applied-gigs" columns={columns} dataSource={shortlistedApplicationData} pagination={false} />
                    </TabPane>
                    <TabPane tab="Selected" key="selected" className="">
                        <Table className="applied-gigs" columns={columns} dataSource={selectedApplicationData} pagination={false} />
                    </TabPane>
                    <TabPane tab="Rejected" key="rejected" className="">
                        <Table className="applied-gigs" columns={columns} dataSource={rejectedApplicationData} pagination={false} />
                    </TabPane>
                </Tabs>
            </div>
        </div>
